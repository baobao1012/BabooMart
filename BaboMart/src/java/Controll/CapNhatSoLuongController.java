 ...
 * @author baobao
 */
@WebServlet(name = "CapNhatSoLuongController", urlPatterns = {"/capnhatsoluong"})
public class CapNhatSoLuongController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if(giohang1.containsKey(Masanpham)){
                giohang1.get(Masanpham).setSoluong(soluong);
            }
            session.setAttribute("giohang1", giohang1);
            response.sendRedirect("giohang1");
        }
    }
